<template>
  <div class="relative flex h-screen w-screen overflow-hidden">
    <div id="logoutDiv" class="hidden absolute w-screen h-screen bg-gray-500/60">
      <div
        class="absolute w-96 h-60 -translate-x-1/2 left-1/2 -translate-y-1/2 top-1/2 border-2 border-violet-950 rounded-3xl"
        style="background-color: rgb(108 13 196);">
        ...
        <div class="w-full h-1/2 grid grid-cols-2 p-5 gap-5">
          <button id="hideLogoutButton" type="button" @click="changeVisibilityLogout"
            class="w-full h-full flex items-center justify-center rounded-xl text-white bg-red-600">No</button>
          <button id="logoutButton" type="button" @click="logout"
            class="w-full h-full flex items-center justify-center rounded-xl text-white bg-green-600">Yes</button>
        </div>
      </div>
    </div>
    <div id="createRoomDiv" class="hidden absolute w-screen h-screen bg-gray-500/60">
      <div
        class="absolute w-96 h-96 -translate-x-1/2 left-1/2 -translate-y-1/2 top-1/2 border-2 border-violet-950 rounded-3xl"
        style="background-color: rgb(108 13 196);">
        ...
        <div class="w-full h-3/4 p-5">
          <div class="w-full h-2/3 pb-5">
            <div class="w-full h-1/3 relative flex justify-center">
              ...
              <label for="nameInput" class="absolute top-0 left-8 text-md rounded-sm"
                style="background-color: rgb(108 13 196);">Name</label>
            </div>
            <div class="hidden w-full h-1/3 relative flex justify-center">
              <div class="h-full w-3/4 pt-5">
                <input name="codeRoom" id="codeRoom" type="text"
                  class="text-black w-full border-2 border-violet-950 rounded-sm" disabled>
              </div>
              <label for="codeRoom" class="absolute top-0 left-8 text-md rounded-sm"
                style="background-color: rgb(108 13 196);">code</label>
            </div>
            <div class="w-full h-1/3 relative flex justify-center">
              <div class="h-full w-3/4 pt-5">
            ...
            <div class="w-full h-1/3 relative flex justify-center">
              ...
              <label for="confirmationPWD" class="absolute top-0 left-8 text-md rounded-sm"
                style="background-color: rgb(108 13 196);">Confirmation of
                Password</label>
            </div>
          </div>
          <div class="w-full h-1/3 grid grid-cols-2 gap-5 pb-6">
            <button type="button" @click="cancelRoomCreation" id="cancelRoomCreation"
              class="h-full w-full border-2 border-violet-950 rounded-md">CANCEL</button>
            <button type="button" @click="createRoom" id="createButton"
              class="h-full w-full border-2 border-violet-950 rounded-md">CREATE</button>
          </div>
        </div>
      </div>
    </div>
    <div id="accessRoomDiv" class="hidden absolute w-screen h-screen bg-gray-500/60">
      <div
        class="absolute w-96 h-96 -translate-x-1/2 left-1/2 -translate-y-1/2 top-1/2 border-2 border-violet-950 rounded-3xl"
        style="background-color: rgb(108 13 196);">
        ...
        <div class="w-full h-3/4 p-5">
          <div class="w-full h-2/3 pb-5">
            <div class="w-full h-1/3 relative flex justify-center">
              <div class="h-full w-3/4 pt-5">
                <input name="codeRoom" id="accessCodeRoom" type="text"
                  class="text-black w-full border-2 border-violet-950 rounded-sm">
              </div>
              <label for="codeRoom" class="absolute top-0 left-8 text-md rounded-sm"
                style="background-color: rgb(108 13 196);">code</label>
            ...
            <div class="w-full h-1/3 relative flex justify-center">
              <div class="h-full w-3/4 pt-5">
                <input name="pwdInput" id="accessPwdInput" type="password"
                  class="text-black w-full border-2 border-violet-950 rounded-sm">
              </div>
              <label for="pwdInput" class="absolute top-0 left-8 text-md rounded-sm"
                style="background-color: rgb(108 13 196);">Password</label>
          ...
          <div class="w-full h-1/3 grid grid-cols-2 gap-5 pb-6">
            <button type="button" @click="cancelRoomAccess" id="cancelRoomAccess"
              class="h-full w-full border-2 border-violet-950 rounded-md">CANCEL</button>
            <button type="button" @click="accessRoom" id="connectButton"
              class="h-full w-full border-2 border-violet-950 rounded-md">CONNECT</button>
          </div>
        </div>
      </div>
    </div>
    <div class="w-1/4 h-full border-r-2 border-violet-950 flex flex-col">
      ...
      <div class="w-full h-16 px-10 grid grid-cols-2 gap-10 justify-center items-center">
        <button @click="openCreateRoomDiv" id="addRoomButton"
          class="h-full font-bold border-2 border-violet-950 rounded-lg" type="button">
          CREATE
        </button>
        <button @click="changeVisibilityAccessRoom" id="accessRoomButton"
          class="h-full font-bold border-2 border-violet-950 rounded-lg" type="button">
          ADD
        </button>
      </div>
    ...
    <div class="w-3/4 h-full flex flex-col">
      <div class="h-16 w-full grid grid-cols-12 items-center pl-5">
        ...
        <div class="col-span-1 w-full h-16 invisible">
          <button class="h-full aspect-square p-2" type="button" id="roomParameterButton">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="white"
              class="w-full h-full rounded-full bg-gray-500">
        ...
        <button type="button" id="profileButton" @click="changeVisibilityLogout"
          class="col-span-3 w-full h-16 flex items-center p-2 border-l-2 border-violet-950 rounded-l-lg">
          <div class="max-w-1/4 h-full aspect-square">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="black"
              class="h-full w-full rounded-full border-2 border-violet-950">
              <path fill-rule="evenodd"
      ...
      <div class="hidden w-full h-fit pr-20 mt-5 text-white flex flex-col" ref="exempleMessage" id="exempleMessage">
        <div
          class="h-10 w-40 text-sm flex items-center font-bold border-2 border-l-0 border-violet-950 bg-violet-600 rounded-r-lg"
          tag="name">Name</div>
        <div
          class="min-w-40 w-fit text-xs flex items-center border-2 border-l-0 border-violet-950 bg-violet-600 rounded-r-lg"
          tag="message">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut vel nisl sem. Nulla orci turpis,
          tempus interdum imperdiet vel, sollicitudin id libero. Sed ac laoreet justo. Sed lacinia sem lacus, ac
          suscipit erat mollis at. Praesent gravida, lectus id elementum commodo, metus libero porta odio, vitae
          volutpat lorem nibh nec erat. Ut tincidunt efficitur enim gravida posuere. Vestibulum id orci congue,
          ultricies elit eu, hendrerit purus. Praesent porttitor erat et suscipit posuere. Nam porttitor ex ante, nec
          ultricies justo pellentesque in. Morbi lobortis posuere mauris, at fringilla odio cursus in. Praesent in nulla
          vel nisl consectetur fringilla id at odio. Suspendisse id ipsum non turpis volutpat aliquet ut a turpis.
          Maecenas quis quam pellentesque, auctor mi ut, dictum eros. Vestibulum sed purus at eros bibendum suscipit.
          Sed leo mauris, bibendum quis neque ac, fringilla auctor urna. Mauris consequat eget ligula sed laoreet.</div>
      </div>
      <div class="w-full h-full overflow-y-auto border-y-2 border-violet-950 flex flex-col" ref="listMessages"
        id="listMessages">
      </div>
      <div class="h-20 w-full grid grid-cols-6 items-center">
        <div class="col-span-5 w-full h-full">
          <textarea id="writtenMessage" @keyup.enter="sendMessage"
            class="text-black w-full h-full resize-none border-r-2 border-violet-950 px-2"
...
if (username != null && username != "" && token != null && token != "") {
  ...
  socket.emit("tokenVerification", { username: username, token: token });

  socket.on("tokenOK", (content) => {
    tokenOK = true;
    document.getElementById("username").textContent = username;
    socket.emit("getListRooms", { username: username, token: token });
    let roomHistory = content.roomHistory;
    if (roomHistory != null && roomHistory != undefined && content.roomHistory != {}) {
  ...
  socket.on("roomAccessible", (content) => {
    // creating the button allowing to join a room
    ...
    roomDiv.addEventListener("click", () => {
      // leaving the current room and joining the next
  ...
  socket.on("roomJoined", (content) => {
    // showing the old messages of the room
    let roomHistory = content.roomHistory;
    if (roomHistory != null && roomHistory != undefined && content.roomHistory != {}) {
      roomHistory.forEach(message => {
        showMessage(message.username, message.message, message.currentUser);
...
// change the visibility on the layout about room creation
...
// change the visibility on the layout about accessing to room
function changeVisibilityAccessRoom() {
  let div = document.getElementById("accessRoomDiv");
  if (div.classList.contains("hidden")) {
    div.classList.remove("hidden");
  }
  else {
...
function openCreateRoomDiv() {
  socket.emit("getNewRoomCode", { username: username, token: token });
}
...
function createRoom() {
  ...
  if (code != null && roomName != null && pwd != null && pwd == confirmationPWD) {
    socket.emit("createNewRoom", { username: username, token: token, code: code, name: roomName, password: pwd });
    // emptying the inputs
    document.getElementById("codeRoom").value = "";
    document.getElementById("nameInput").value = "";
    document.getElementById("pwdInput").value = "";
...
function cancelRoomAccess() {
  document.getElementById("accessCodeRoom").value = "";
  document.getElementById("accessPwdInput").value = "";
  changeVisibilityAccessRoom();
}

function accessRoom() {
  let code = document.getElementById("accessCodeRoom").value;
  let pwd = encryption(document.getElementById("accessPwdInput").value);
  if (code != null && pwd != null) {
    socket.emit("accessNewRoom", { username: username, token: token, code: code, password: pwd });
    cancelRoomAccess();
  }
}
</script>
